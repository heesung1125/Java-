package com.Package.shootingGame1;

import javax.swing.*;
import java.awt.*;
import java.awt.event.*;
import java.util.*;
//import java.io.*; // 소스 변경으로 삭제
//import javax.imageio.*; // 소스 변경으로 삭제
...
class game_Frame extends JFrame implements KeyListener, Runnable {
	...
	public void init() {
		...
		Missile_img = new ImageIcon("imge/Missile.png").getImage();
		Enemy_img = new ImageIcon("imge/enemy.png").getImage();
		// 이미지 만드는 방식을 ImageIcon으로 변경.

		Player_img = new Image[5];
		for (int i = 0; i < Player_img.length; ++i) {
			Player_img[i] = new ImageIcon("imge/f15k_" + i + ".png").getImage();
		}
		// 플레이어 애니메이션 표현을 위해 파일이름을
		// 넘버마다 나눠 배열로 담는다.

		BackGround_img = new ImageIcon("imge/background.png").getImage();
		// 전체 배경화면 이미지를 받습니다.

		Cloud_img = new Image[3];
		for (int i = 0; i < Cloud_img.length; ++i) {
			Cloud_img[i] = new ImageIcon("imge/cloud_" + i + ".png").getImage();
		}
		// 구름을 3개 동시에 그리는데 편의상 배열로 3개를 동시에 받는다.

		Explo_img = new Image[3];
		for (int i = 0; i < Explo_img.length; ++i) {
			Explo_img[i] = new ImageIcon("imge/explo_" + i + ".png").getImage();
		}
		// 폭발 애니메이션 표현을 위해
